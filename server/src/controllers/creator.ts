...
import { sendCreateCertificateEmail, sendLoginEmail, sendMintEmail, sendRegisterEmail, sendWithdrawEmail } from "../utils/sendEmail";
...
const creatorLogin = async (req: Request, res: Response) => { 
  try {
    ...
    if (!userExists) {
      res.status(httpStatus.BAD_REQUEST).json({ error: "Invalid credentials or User does not exist" });
      return;
    }
...
const history = async (req: Request, res: Response) => { 
  try {
    ...
    const historyInfo = await certificateModel.find({ owner: email }).select("-_id");
    if (!historyInfo) {
      res.status(httpStatus.OK).json({ message: "Nothing found", historyInfo: "" });
      return;
    }
    ...
    console.log({ users });
    res.status(httpStatus.OK).json({ message: "History information sent", historyInfo: users });
  } catch (error: any) {
...
const getRevenue = async (req: Request, res: Response) => {
  try {
    ...
    res.status(httpStatus.OK).json({ message: "Total revenue fetched", revenue: revInfo?.totalRevenue })
  } catch (error: any) {
...
const getWallet = async (req: Request, res: Response) => {
  try {
    ...
    res.status(httpStatus.OK).json({ message: "user found", wallet: walletFetch.walletAddress })
  } catch (error: any) {
...
const setWallet = async (req: Request, res: Response) => {
  try {
    ...
    res.status(httpStatus.OK).json({ message: "wallet setup successfully" })
  } catch (error: any) {
...
const createEmail = async (req: Request, res: Response) => {
  try {
    ...
    await sendCreateCertificateEmail(emailProp, "🎉 Certificate Created", txUrl);
    res.status(httpStatus.CREATED).json({ message: "Create certificate email sent" });
  } catch (error: any) {
...
const withdrawEmail = async (req: Request, res: Response) => {
  try {
    ...
    await sendWithdrawEmail(emailProp, "Wallet has been successfully withdrawn", options);
